...
def plot_history_metrics(history: keras.callbacks.History):
    ...
    for i, (key, value) in enumerate(history.history.items()):
        ...
        plt.title(str(key))
    plt.savefig('lstm_history_metrics.png')
...
def create_lstm_model():
    ...
    #bidirectional LSTMs
    x = layers.Bidirectional(layers.LSTM(128, return_sequences=True, activation="tanh"))(input_layer)
...
callbacks = [
    keras.callbacks.ModelCheckpoint(
        "best_model_lstm.keras", save_best_only=True, monitor="loss"
    ),
...
def view_evaluated_eeg_plots(model):
    ...
    plt.savefig('lstm_evaluated_eeg_plots.png')
